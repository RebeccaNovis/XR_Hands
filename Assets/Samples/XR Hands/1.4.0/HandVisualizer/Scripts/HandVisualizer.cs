    ...
    public class HandVisualizer : MonoBehaviour
    {
        ...
        /*public void Start()
        {
            GetComponent<ChuckSubInstance>().RunCode(@"
                fun void harmonic(int harmNum)
                {
                    SinOsc sin => dac;
                    Math.random2f(.5,5.0) => float vibRate;
                    while( true )
                    {
                        harmNum * 110. => sin.freq;
                        ( 1 + Math.sin( now/second * Math.PI * vibRate )) * .05 => sin.gain;
                        sin.gain() + (.3 / harmNum) => sin.gain;
                        10::ms => now;
                    }
                }
            ");
    ...
    public bool debugDrawJoints
        ...
        void OnUpdatedHands(XRHandSubsystem subsystem, XRHandSubsystem.UpdateSuccessFlags updateSuccessFlags, XRHandSubsystem.UpdateType updateType)
        {
            ...
            if (updateType == XRHandSubsystem.UpdateType.Dynamic)
            {
                ...
                Debug.Log("rightHand Pos: " + wristPose.position);
